  ...
  let renderiza = function(){
    ...
    for(var i = 0; i < data.length; i++){
      ...
      for(var r = 0; r < conversa_data.length; r++){
        ...
        var pre_nome = conversa_data[r].split(/\d{2}\/\d{2}\/\d{2}\s\d{2}\:\d{2}/g)[0];
        console.log('pre_nome:' + pre_nome);
        var nome = "";
        console.log();
        // console.log(pre_nome.match(/\s\-\s.*\:/g)[0]);
        if (pre_nome.match(/\s\-\s.*\:/g)!=null && pre_nome.match(/\s\-\s.*\:/g)!="null") {
          nome = pre_nome.match(/\s\-\s.*\:/g)[0];
        }else{
          nome = "";
        }
